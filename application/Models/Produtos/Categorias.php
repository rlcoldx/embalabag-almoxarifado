...
class Categorias extends Model
{
    ...
    {
        $this->read = new Read();
        $this->read->FullRead("SELECT p.* FROM produtos AS p LEFT JOIN produtos_categorias AS pc ON pc.id_produto = p.id WHERE pc.id_produto IS NULL");
        return $this->read;
    }
    ...
    public function editarCategory($params): Update
    {
        ...
        $update_produtos_categorias = new Update();
        $update_produtos_categorias->ExeUpdate('produtos_categorias', $params, 'WHERE `id_categoria` = :id', "id={$id}");
    ...
    {
        $this->read = new Read();
        $this->read->FullRead("SELECT * FROM produtos_categorias WHERE id_categoria = :id_categoria", "id_categoria={$id_categoria}");
        ...
        foreach ($categoria_dados_lista_produtos as $categoria_lista_produtos) {
            ...
            $this->read->FullRead("SELECT * FROM produtos_categorias WHERE id_produto = :id_produto", "id_produto={$categoria_lista_produtos['id_produto']}");
            ...
            $this->read->FullRead("UPDATE produtos SET categoria_id = :categoria_id, categoria = :categoria WHERE id = :id_produto", "categoria_id={$categorias_ids}&categoria={$categorias_nomes}&id_produto={$categoria_lista_produtos['id_produto']}");
        }
    }
}